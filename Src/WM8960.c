...
I2C_HandleTypeDef* hc;

I2S_HandleTypeDef* hs;
...
uint8_t WM8960_Write_Reg(uint8_t reg, uint16_t dat)  {
  ...
  res = HAL_I2C_Master_Transmit(hc,(WM8960_ADDRESS<<1),I2C_Data,2,10);
  if(res == HAL_OK)
...
/**
  ...
  * @retval None
  */
uint8_t WM89060_Init(I2C_HandleTypeDef* c, I2S_HandleTypeDef* s , uint16_t volume)  {
  ...
  //WM8960_Write_Reg(0x29, 0x007F | 0x0100); //Right Speaker Volume
	WM8960_Write_Reg(0x28, volume | 0x0100); //Left Speaker Volume
	WM8960_Write_Reg(0x29, volume | 0x0100); //Right Speaker Volume
...
uint32_t AUDIO_Play(uint16_t* pBuffer, uint32_t Size)
{
  ...
  res = HAL_I2S_Transmit_DMA(hs,pBuffer, (uint32_t)(DMA_MAX(Size/2)));
...
//	temp = rand() % 65000;
	...
	HAL_StatusTypeDef sd_to_decoder = HAL_I2S_Transmit(hs,(uint16_t*)temp, 1, 2000);
