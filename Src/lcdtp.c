...
void LCD_DrawRectangle ( uint16_t usX_Start, uint16_t usY_Start, uint16_t usWidth, uint16_t usHeight, uint8_t ucFilled ,  uint16_t usColor)
{
	if ( ucFilled )
	{
...
void LCD_DrawCircle ( uint16_t usC, uint16_t usP, uint16_t R, uint16_t usColor , int filled)
{	
	int16_t sCurrentX, sCurrentY;
	int16_t sError;
	
	
	sCurrentX = 0; sCurrentY = R;	  
	...
	while ( sCurrentX <= sCurrentY )
	{
		...
		if ( filled ) 			
			...
			{                      
				LCD_DrawDot ( usC + sCurrentX, usP + sCountY , usColor);          
				LCD_DrawDot ( usC - sCurrentX, usP + sCountY ,usColor);           
				LCD_DrawDot ( usC - sCountY,   usP + sCurrentX ,usColor);         
				LCD_DrawDot ( usC - sCountY,   usP - sCurrentX ,usColor);         
				LCD_DrawDot ( usC - sCurrentX, usP - sCountY ,usColor);           
        LCD_DrawDot ( usC + sCurrentX, usP - sCountY ,usColor);         
				LCD_DrawDot ( usC + sCountY,   usP - sCurrentX ,usColor);         
		...
		{          
			LCD_DrawDot ( usC + sCurrentX, usP + sCurrentY ,usColor);             
			LCD_DrawDot ( usC - sCurrentX, usP + sCurrentY ,usColor);           
			LCD_DrawDot ( usC - sCurrentY, usP + sCurrentX ,usColor);            
			LCD_DrawDot ( usC - sCurrentY, usP - sCurrentX ,usColor);            
			LCD_DrawDot ( usC - sCurrentX, usP - sCurrentY ,usColor);                   
			LCD_DrawDot ( usC + sCurrentX, usP - sCurrentY ,usColor);            
			LCD_DrawDot ( usC + sCurrentY, usP - sCurrentX ,usColor);            
...
void LCD_DrawRectangle_Text( uint16_t usX_Start, uint16_t usY_Start, uint16_t usWidth, uint16_t usHeight, uint8_t ucFilled ,  uint16_t usColor , const char * pStr ){
	LCD_DrawRectangle( usX_Start,  usY_Start,  usWidth,  usHeight, ucFilled , usColor);
	LCD_DrawString( (usX_Start + usWidth) / 2, (usY_Start + usHeight) / 2, pStr );
}
